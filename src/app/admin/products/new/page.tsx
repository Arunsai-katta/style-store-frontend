...
export default function AddProductPage() {
  ...
  const handleImageUpload = async (variantIndex: number, files: File[]) => {
    ...
    // Prevent duplicate uploads if somehow called concurrently
    if (uploadingImages[uploadKey]) {
      return;
    }
                ...
                <div className="mb-4">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (!files || files.length === 0) return;
                      // Snapshot files into an array immediately, then reset the input.
                      // This prevents React Strict Mode's double-invoke from uploading twice.
                      const fileArray = Array.from(files);
                      e.target.value = '';
                      handleImageUpload(variantIndex, fileArray);
                    }}
                    className="hidden"
                  />
